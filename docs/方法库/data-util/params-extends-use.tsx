...
const initialValues = { arr: { a: { a: 'a' }, b: { b: 'b' } , c: '' }, item: [{ item: { 'a.b' : 'ab'}}, { item: { 'b.c' : 'cc'}}]};
...
const Demo = () => {
  ...
  const onClick = () => {
    ...
    const rs = DataUtil.params.extends(toJSON(arr), toJSON(item));
    setResult(rs);
  }
  ...
  return (<CollapsibleCard title="示例" defaultCollapsed={true}>
    <YForm form={form} initialValues={initialValues}>
      {[
        ...
        { label: 'args  (参数2)', type: 'codeEditorCard', name: 'item' },
        {
          type: 'space',
          items: [
